...
from models.flex_gcn import FlexGCN
...
# 2. Load model
model['flex_gcn'] = FlexGCN(adj, opt.hid_dim, dim=1, beta = opt.beta, num_layers=opt.num_layers, p_dropout=p_dropout).cuda()
...
# 4. Reload model
flex_gcn_dict = model['flex_gcn'].state_dict()

if opt.flex_gcn_reload == 1: 
    
    pre_dict_flex_gcn = torch.load(os.path.join(opt.previous_dir, opt.module_flex_model))
    # For name, key in stgcn_dict.items():
    for name, key in flex_gcn_dict.items(): 
        if name.startswith('A') == False:
           
            flex_gcn_dict[name] = pre_dict_flex_gcn[name]
    
    model['flex_gcn'].load_state_dict(flex_gcn_dict)
...
for epoch in range(1, opt.nepoch):
    ...
    if opt.pro_test == 1:
        ...
        if opt.save_model and data_threshold < opt.previous_best_threshold:
            opt.previous_flex_gcn_name = save_model(opt.previous_flex_gcn_name, opt.save_dir, epoch, opt.save_out_type, data_threshold, model['flex_gcn'], 'flex_gcn')
