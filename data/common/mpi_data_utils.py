...
# import random
# # from mpii_full import DataGenerator_mpii
# import utils
...
# def create_2d_mpii(Debug=False):

#   mpii_to_human36 = np.array([6, 2, 1, 0, 3, 4, 5, 7, 8, 9, 13, 14, 15, 12, 11, 10])

#   data_file = '/home/lichen/pose_estimation/3d-pose-baseline-master/data/mpii/'
#   input_file = '{}/train.h5'.format(data_file)
#   annot_train = getData(input_file)
#   joints = annot_train['part']
#   names = annot_train['name']
...
#   train_set, data_mean, data_std = normalize_data_mpii(complete_train, dim_to_use)
...
#   if dataset == 'mpi':
#     input_file = '/home/nies_14/Desktop/Research/Pose With GCN/Mine/Mine-With-MPI/mpi_test/data_300/annotVal_500_green.h5'
#     annot_train = getData(input_file)
...
#     data_file = '/home/lichen/pose_estimation/mpii_sh'
#     input_file = '{}/{}_sh_completeJoints_256.h5'.format(data_file, dataset)
#     annot_train = getData(input_file)
#     joints = annot_train['part']
...
#   joints_ori = copy.deepcopy(joints)
#   root = joints[:, 0:1, :]
...
#   h36_to_mpii_permu = np.array([3, 2, 1, 4, 5, 6, 0, 7, 8, 9, 15, 14, 13, 10, 11, 12])
...
#   #
#   # data_file = '/home/lichen/pose_estimation/mpii_sh'
#   # input_file = '{}/mpii_sh_completeJoints_256.h5'.format(data_file)
#   # annot_train = getData(input_file)
#   # joints_3d = annot_train['joints_3d']
...
# def getTrainingData():
#   with h5py.File('/home/lichen/pose_estimation/3d-pose-baseline-master/experiments/mdm_train_data/data_mdm_top4.h5','r') as f:

#     enc_in  = f['all_poses_top4'].value
#     dec_out = f['all_dec_out'].value

#   return enc_in, dec_out


# def getValidData(action):
#   with h5py.File('/home/lichen/pose_estimation/3d-pose-baseline-master/experiments/mdm_train_data/data_mdm_valid/{}_top4.h5'.format(action),'r') as f:
...
#   joints_missing = [2, 3, 5, 6, 11, 12, 14, 15]
...
#   # action = 'all'
#   # data_dir = "/home/lichen/pose_estimation/3d-pose-baseline-master/data/h36m/"
#   # actions = define_actions(action)
#   #
#   # train_set_2d, test_set_2d, data_mean_2d, data_std_2d, dim_to_ignore_2d, dim_to_use_2d = read_2d_predictions(actions, data_dir)


#   #### debug data process for mpii data which is generated by hourglass ####


#   # batchsize = 8
#   # trainset_2d, mean, std, dim_to_use = create_2d_mpii_test(Debug=False)
#   # enc_in = get_all_batches_mpii(trainset_2d, batchsize)
#   # data_gen = DataGenerator_mpii()
#   # data_gen.generateSet(filter=False)
#   # validation_sizes = data_gen.data_sizes['valid']
#   # generator_source = data_gen._aux_generator(batchsize, sample_set='valid')
#   #
#   # for i in range(len(enc_in)):
#   #   train_img, _, _, _, _, _ = next(generator_source)
#   #   input = enc_in[i]
#   #   un = unnormalize_data_mpii(input, mean, std, dim_to_use)
#   #   un = un[:, dim_to_use]
#   #   pose_mpii = h36_to_mpii(un)
#   #   for j in range(pose_mpii.shape[0]):
#   #     dst = utils.draw_result(train_img[j], pose_mpii[j, :, :])
#   #     cv2.imshow('img', dst)
#   #     cv2.waitKey()
...
#   input_file = '/home/lichen/pose_estimation/pose_estimation_DA/data_300/annotVal_500.h5'
#   data = h5py.File(input_file, 'r')
#   joints_3d = data['annot_3d'].value

#   test_set, data_mean, data_std, dim_to_ignore, dim_to_use, test_root_positions, pose_mpi_to_h36 = create_3d_mpi_test(joints_3d)

#   ori = unNormalizeData(test_set, data_mean, data_std, dim_to_ignore)

#   diff = (ori + np.tile(test_root_positions, [1, len(H36M_NAMES)]))[:, dim_to_use]-pose_mpi_to_h36.reshape(joints_3d.shape[0], -1)[:, 3:]
